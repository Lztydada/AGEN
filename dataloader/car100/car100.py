...
class CAR100(Dataset):
    ...
    def _pre_operate(self, root):
        image_file = os.path.join(root, 'stanford_cars/images.txt')
        split_file = os.path.join(root, 'stanford_cars/train_test_split.txt')
        class_file = os.path.join(root, 'stanford_cars/image_class_labels.txt')
        ...
        if self.train:
            for k in train_idx:
                image_path = os.path.join(root, 'stanford_cars/images', id2image[k])
